...
/*function addBinding(env, name, value) {
	//Not needed ?? If env[name] then don't overwrite but addBinding only called if !env[name]
	// Object.keys(env).forEach(key => { env[key] = env[key].rewrite({name: value}); });
	env[name] = value;
}*/
...
const [X, Y] = vars('X', 'Y'),
	builtinRules = [
		rule(clause(function notEqual([arg1, arg2]) {
			return (arg1 instanceof Var) || (arg2 instanceof Var) || (arg1.toString() != arg2.toString());
		}, [X, Y])),
